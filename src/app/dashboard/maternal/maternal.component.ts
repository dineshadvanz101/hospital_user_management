...
export class MaternalComponent implements OnInit, OnChanges {
  ...
  calculateBMI() {
    ...
    if(this.maternalForm.value["mother_height"].length >0) {
  ...
  maternalFormSubmit() {
    ...
    const newUser = vim.common_api.maternal_add(vim.maternalForm.value);
    newUser.subscribe(
      response => {
        vim.reset();
  ...
  update_maternal_form() {
    ...
    vim.common_api.updateMaternalProfile(vim.id, vim.maternalForm.value)
    .subscribe(result => {
